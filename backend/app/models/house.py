import enum
from sqlalchemy import String, Integer, Float, ForeignKey, Boolean, Numeric, Text, Enum
...
class ManagementType(str, enum.Enum):
    UK = "uk"       # Управляющая организация
    TSZH = "tszh"   # Товарищество собственников жилья


class ProviderCategory(str, enum.Enum):
    HEATING_HOT_WATER = "heating_hot_water"  # Отопление и ГВС (УСТЭК)
    COLD_WATER = "cold_water"                # ХВС и водоотведение (Водоканал)
    ELECTRICITY = "electricity"              # Электроснабжение (ЭК Восток)
    TELECOM = "telecom"                      # Интернет и домофония (Дом.ру, Мегафон)
    MUNICIPAL = "municipal"                  # Муниципальный контроль (ДГХ)


class House(Base, TimestampMixin):
    """Многоквартирный дом (паспорт БТИ, техплан, конструктив)"""
    __tablename__ = "houses"

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    address: Mapped[str] = mapped_column(String(255), nullable=False, index=True)  # ул. Баумана, д. 12
    city: Mapped[str] = mapped_column(String(100), default="Казань", nullable=False)
    district: Mapped[str] = mapped_column(String(150), nullable=False)  # Вахитовский район
    ...
    wear_percentage: Mapped[int] = mapped_column(Integer, default=23, nullable=False)  # 23%
    total_area: Mapped[float] = mapped_column(Float, default=4116.0, nullable=False)   # 4 116 м²
    living_area: Mapped[float] = mapped_column(Float, default=3253.9, nullable=False)  # 3 253.9 м²
    floors: Mapped[int] = mapped_column(Integer, default=10, nullable=False)          # 10 этажей
    entrances: Mapped[int] = mapped_column(Integer, default=4, nullable=False)        # 4 подъезда
...
class Apartment(Base, TimestampMixin):
    ...
    personal_account: Mapped[str] = mapped_column(String(30), unique=True, index=True, nullable=False)  # "8492-3019-44"
...
class HouseServiceProvider(Base):
    """Справочник РСО и подрядчиков дома (УСТЭК, Водоканал, Дом.ру и т.д.)"""
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)  # АО «УСТЭК», ООО «Тюмень Водоканал»
    service_description: Mapped[str] = mapped_column(String(255), nullable=False)  # Отопление и ГВС
    phone: Mapped[str | None] = mapped_column(String(50), nullable=True)
    brand_badge: Mapped[str | None] = mapped_column(String(50), nullable=True)  # "Дом.ру", "FTTB"
